...
// from the admin dashboard OR a Telegram button. Keeps the Telegram forum in sync
// with the `orders` table. NEVER changes orders.order_status, so it can't loop.
// Auth: x-sync-secret header == Vault telegram_sync_secret (constant-time).
...
Deno.serve(async (req) => {
  ...
  if (firstPost) {
    ...
    if (!deleted && mainId != null) await stripButtons(token, chatId, mainId);
    await del(token, chatId, proofId);
    const newMain = await sendCard(token, chatId, targetThread, text, keyboard);
    if (newMain == null) return json({ ok: false, error: "card send failed" }, 502);
    mainId = newMain;
    proofId = order.payment_proof_url ? await sendProof(token, chatId, targetThread, order.payment_proof_url, proofCaption) : null;
    threadNow = targetThread;
  }
